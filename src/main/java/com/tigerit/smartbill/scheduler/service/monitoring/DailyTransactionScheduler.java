...
import static com.tigerit.smartbill.common.values.APIValues.BillingServer.Scheduler.GENERATE_REPORT_DAILY;

@Slf4j
@Component
public class DailyTransactionScheduler {
	
	public boolean RunnableDailyTransaction() {
        log.info ("Entering Daily Report generation task , Start Time " + LocalDate.now ());

        Object payload = null;
        
        String fullGET_URL = APIValues.BillingServer.Scheduler.SCHEDULER_CONTROLLER_BASE_PATH + GENERATE_REPORT_DAILY;
        ...
        try {
            ...
            payload=false;
            log.info ("[Daily Report Generation Error] " + ex.getMessage ());
        }
